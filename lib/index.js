...
export class rpio extends EventEmitter {
  ...
  initialize = (options) => {
    ...
    if (this.options.mock) {
      ...
      if (!!detected.pinmap) {
        ...
        this.emit('warn', `Hardware auto-detection failed (${JSON.stringify(detected)}), running in ${rpio.defaultMock} mock mode`);
  ...
  */
  ...
  setPinPwm(pin) {
    ...
    this.maybeMock(binding.gpio_function, gpioPin, pwmPin.function);
    this.maybeMock(binding.pwm_set_mode, pwmPin.channel, 1, 1);
  }
  ...
  i2cEnd = () => {
    this.maybeMock(binding.i2c_end);
  }
  ...
  spiBegin = () => {
    ...
    this.maybeMock(binding.spi_begin);
  }
  ...
  spiEnd = () => {
    this.maybeMock(binding.spi_end);
  }

  sleep = (seconds) => {
    this.maybeMock(binding.rpio_usleep, seconds * 1000000);
  }

  msleep = (milliseconds) => {
    this.maybeMock(binding.rpio_usleep, milliseconds * 1000);
  }

  usleep = (picoseconds) => {
    this.maybeMock(binding.rpio_usleep, picoseconds);
  }
}
